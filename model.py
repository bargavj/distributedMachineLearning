...
#4 for Regression and 2 for Logistic

class model:
    #### Baseline 1 ####
    def baseline1(X, y, lambda2, epsilon): # Chaudhuri et al. 2011 - Output Perturbation
        n, d = X.shape[0], X.shape[1]
        clf = linear_model.Ridge(alpha = lambda2).fit(X, y) # TRY DIFFERENT SOLVER LIKE LBFGS
    ...
    def baseline11(X, y, lambda2, epsilon): # Chaudhuri et al. 2011 - Objective Perturbation
        n, d = X.shape[0], X.shape[1]        
        ...
        def log_reg_grad(v):
            l = np.exp(np.dot(X, v))
            #return -np.dot(X.T, 1. / (1 + l)) + lambda2 * v
            return (np.dot(XtX, v) - Xty) / n + lambda2 * v

        beta = apg.solve(log_reg_grad, {}, np.zeros(d), eps=1e-12, quiet=True) 
        beta += np.random.laplace(0, 2. / (n * lambda2 * epsilon), d)
        return beta
    ...
    def baseline2(X, y, lambda2, epsilon): # Chaudhuri et al. 2011 - Objective Perturbation
        ...
        def log_reg_grad(v):
            l = np.exp(np.dot(X, v))
            #return -np.dot(X.T, 1. / (1 + l)) + (Delta + lambda2) * v + b / n # divided remaining terms by n
            return (np.dot(XtX, v) - Xty) / n + (Delta + lambda2) * v + b / n

        beta = apg.solve(log_reg_grad, {}, np.zeros(d), eps=1e-12, quiet=True) # fix step size - might help in better accuracy with noise
        return beta
